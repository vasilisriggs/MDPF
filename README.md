...
και αντιγράφω το reference του σε ένα αντικείμενο ίδιου τύπου.

Βρίσκω το τελευταίο κλειδί που υπάρχει στο φύλλο **leaf** και το αποθηκεύω σε μία μεταβλητή

	upper = leaf.getLastKey();
	
που αποτελεί το άνω φράγμα του εύρους.

Τέλος, βρίσκω και τη χωριτικότητα του φύλλου, δηλαδή τον συνολικό αριθμό των **indices** που υπάρχει στο φύλλο
	
	capacity = leaf.getCurrentCapacity();
και την αποθηκεύω σε μία μεταβλητή.

Τέλος, αποθηκεύω τις μεταβλητές **upper**, **lower** και **capacity** σε ένα αρχείο **LeafElements**,
	
	writer.append(chars+" "+String.valueOf(capacity)+" "+String.valueOf(lower)+"-"+String.valueOf(upper));
και 

	lower = upper+1; // go next range
πηγαίνω στο επόμενο φύλλο **leaf**.
